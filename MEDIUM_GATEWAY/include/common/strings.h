...
#ifndef _INC_CC_STRINGS_H
#define _INC_CC_STRINGS_H
...
static force_inline int inc_strlen_zero(const char *s)
{
	return (!s || (*s == '\0'));
}
 ...
 */
#define S_OR(a, b)	(!inc_strlen_zero(a) ? (a) : (b))

/*!
  \brief Gets a pointer to the first non-whitespace character in a string.
  \param inc_skip_blanks function being used
  \param str the input string
  \return a pointer to the first non-whitespace character
 */
INC_INLINE_API(
char *inc_skip_blanks(const char *str),
{
...
/*!
  ...
  \param inc_trim_blanks function being used
  \param str the input string
  \return a pointer to the modified string
 */
INC_INLINE_API(
char *inc_trim_blanks(char *str),
{
	char *work = str;
...
/*!
  ...
  \param inc_skip_noblanks function being used
  \param str the input string
  \return a pointer to the first whitespace character
 */
INC_INLINE_API(
char *inc_skip_nonblanks(char *str),
{
...
/*!
  ...
  \param inc_copy_string function being used
...
*/
INC_INLINE_API(
void inc_copy_string(char *dst, const char *src, size_t size),
{
	while (*src && size) {
		*dst++ = *src++;
		size--;
...
*/
int inc_build_string(char **buffer, size_t *space, const char *fmt, ...) __attribute__((format(printf, 3, 4)));
...
*/
int inc_build_string_va(char **buffer, size_t *space, const char *fmt, va_list ap) __attribute__((format(printf, 3, 0)));
...
/* The realloca lets us inc_restrdupa(), but you can't mix any other inc_strdup calls! */
 ...
 */
static force_inline int inc_str_hash(const char *str)
{
	int hash = 5381;
