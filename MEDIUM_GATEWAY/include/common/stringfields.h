/*! \file
  ...
  struct sample_fields {
	  int x1;
	  INC_DECLARE_STRING_FIELDS(
		  INC_STRING_FIELD(name);
		  INC_STRING_FIELD(address);
		  INC_STRING_FIELD(password);
	  );
	  long x2;
  };
  \endcode
  ...
  if (sample) {
	  if (inc_string_field_init(sample, 256)) {
		  free(sample);
		  sample = NULL;
	  }
  }
  ...
  revert to that when inc_string_field_free() is called. This means
  ...
  Storing simple values into fields can be done using inc_string_field_set();
  more complex values (using printf-style format strings) can be stored
  using inc_string_field_build().
  
  When the structure instance is no longer needed, the fields
  and their storage pool must be freed:
  
  \code
  inc_string_field_free_memory(sample);
  free(sample);
  \endcode
*/

#ifndef _INC_CC_STRINGFIELDS_H
#define _INC_CC_STRINGFIELDS_H
...
/*!
  ...
  Don't declare instances of this type directly; use the INC_STRING_FIELD()
  macro instead.
*/
typedef const char * inc_string_field;

/*!
  \internal
  \brief A constant empty string used for fields that have no other value
*/
extern const char __inc_string_field_empty[];

/*!
  \internal
  \brief Structure used to hold a pool of space for string fields
*/
struct inc_string_field_pool {
	struct inc_string_field_pool *prev;	/*!< pointer to the previous pool, if any */
...
*/
struct inc_string_field_mgr {
	struct inc_string_field_pool *pool;	/*!< the address of the pool's structure */
...
*/
#define INC_STRING_FIELD(name) const inc_string_field name

/*!
  \brief Declare the fields needed in a structure
  \param field_list The list of fields to declare, using INC_STRING_FIELD() for each one
*/
#define INC_DECLARE_STRING_FIELDS(field_list) \
	inc_string_field __begin_field[0]; \
	field_list \
	inc_string_field __end_field[0]; \
	struct inc_string_field_mgr __field_mgr
...
*/
#define inc_string_field_count(x) \
	(offsetof(typeof(*(x)), __end_field) - offsetof(typeof(*(x)), __begin_field)) / sizeof(inc_string_field)
...
/*!
  ...
  array of fields
*/
#define inc_string_field_index(x, field) \
	(offsetof(typeof(*x), field) - offsetof(typeof(*x), __begin_field)) / sizeof(inc_string_field)
...
/*!
  ...
  \return 0 on success, non-zero on failure
*/
#define inc_string_field_init(x, size) \
	__inc_string_field_init(&(x)->__field_mgr, size, &(x)->__begin_field[0], inc_string_field_count(x))
...
/*!
  ...
  \return nothing
*/
#define inc_string_field_index_set(x, index, data) do { \
    char *__zz__ = (char*) (x)->__begin_field[index]; \
    char *__data__ = (char*) data; \
    size_t __dlen__ = strlen(__data__) + 1; \
    if ( __dlen__ == 1 ) {\
      (x)->__begin_field[index] = __inc_string_field_empty; \
    } else { \
      if ((__zz__[0] != 0) && (__dlen__ <= (strlen(__zz__) + 1))) { \
        memcpy(__zz__, __data__, __dlen__); \
      } else { \
        if (((x)->__begin_field[index] = __inc_string_field_alloc_space(&(x)->__field_mgr, __dlen__, &(x)->__begin_field[0], inc_string_field_count(x)))) { \
...
#define inc_string_field_index_logset(x, index, data, logstr) do { \
    char *__zz__ = (char*) (x)->__begin_field[index]; \
    char *__data__ = (char*) data; \
    size_t __dlen__ = strlen(__data__) + 1; \
    if ( __dlen__ == 1 ) {\
      ...
      if ((__zz__[0] != 0) && (__dlen__ <= strlen(__zz__) + 1)) { \
        inc_verbose("%s: ======replacing '%s' with '%s'\n", logstr, __zz__, __data__); \
        memcpy(__zz__, __data__, __dlen__); \
      } else { \
        if (((x)->__begin_field[index] = __inc_string_field_alloc_space(&(x)->__field_mgr, __dlen__, &(x)->__begin_field[0], inc_string_field_count(x)))) \
          inc_verbose("%s: ++++++allocating room for '%s' to replace '%s'\n", logstr, __data__, __zz__); \
...
/*!
  ...
  \return nothing
*/
#define inc_string_field_set(x, field, data) \
	inc_string_field_index_set(x, inc_string_field_index(x, field), data)

#define inc_string_field_logset(x, field, data, logstr) \
	inc_string_field_index_logset(x, inc_string_field_index(x, field), data, logstr)
...
/*!
  ...
  \return nothing
*/
#define inc_string_field_index_build(x, index, fmt, args...) \
	__inc_string_field_index_build(&(x)->__field_mgr, &(x)->__begin_field[0], inc_string_field_count(x), index, fmt, args)
...
/*!
  ...
  \return nothing
*/
#define inc_string_field_index_build_va(x, index, fmt, args1, args2) \
	__inc_string_field_index_build_va(&(x)->__field_mgr, &(x)->__begin_field[0], inc_string_field_count(x), index, fmt, args1, args2)
...
/*!
  ...
  \return nothing
*/
#define inc_string_field_build(x, field, fmt, args...) \
	inc_string_field_index_build(x, inc_string_field_index(x, field), fmt, args)
...
/*!
  ...
  \return nothing
*/
#define inc_string_field_build_va(x, field, fmt, args1, args2) \
	inc_string_field_index_build_va(x, inc_string_field_index(x, field), fmt, args1, args2)
...
*/
#define inc_string_field_index_free(x, index) do { \
	(x)->__begin_field[index] = __inc_string_field_empty; \
	} while(0)
...
/*!
  ...
  pointer is just changed to point to an empty string.
*/
#define inc_string_field_free(x, field) \
	inc_string_field_index_free(x, inc_string_field_index(x, field))
...
*/
#define inc_string_field_free_memory(x) do { \
	struct inc_string_field_pool *this, *prev; \
...
/*!
  ...
  stringfields again.
*/
#define inc_string_field_reset_all(x) do { \
	int index; \
	for (index = 0; index < inc_string_field_count(x); index++) \
		inc_string_field_index_free(x, index); \
	(x)->__field_mgr.used = 0; \
	(x)->__field_mgr.space = (x)->__field_mgr.size; \
	} while(0)

#endif /* _INC_CC_STRINGFIELDS_H */
