...
inline void dump_buf(unsigned char* str, const unsigned char* buf,int count)
{
	int i;
...
int zwjrpa_cb(unsigned char *buf, int len) {
	...
	DevType = buf[0];
	DevId = (buf[2] << 8) | buf[1];  //ntohs(*(unsigned short *) &buf[1]);
...
#endif
#if 0
	Board_Mng_716_Set_Dev_Id(0x14, 0x1234);
	Board_Mng_716_Set_Army_Id(0x0987);
	Board_Mng_716_Set_Num_Len(6);
	Board_Mng_716_Set_Speech_Encode(4);	
#endif
	return 0;
}
...
Board_Mng_Set_Inf_Type(0xd, 2);
Board_Mng_Set_Inf_Speed(0xd, 0x08);
Board_Mng_Set_Inf_Mode(0xd, 0x2);
Board_Mng_Set_Inf_Clk(0xd, 2);
Board_Mng_Set_Inf_SM(0xd, 2);
Board_Mng_Set_Inf_Huanhui(0xd, 1);
...
int Board_Mng_Set_User_Num(uint8 CmdId, uint8 OpMode, uint32 UserNum, uint16 SecurityNum)
{
	ST_HF_MGW_DCU_PRO Regmsg;
	MNG_ZW_SET_USERNUM ZxCmd;
	int len = 0;
	...
	ZxCmd.CmdLen = 0x21;
	ZxCmd.CmdId = CmdId;
	ZxCmd.UserNum = htonl(UserNum);
	ZxCmd.SecurityNum = htons(SecurityNum);
	...
	printf("%s CmdId=%d, OpMode=0x%x, UserNum=0x%x, SecurityNum=0x%x\n", __func__, CmdId, OpMode, UserNum, SecurityNum);
...
int usrnum_cb(unsigned char *buf, int len) {
	...
	CmdId = buf[0];
	UserNum = ntohl(*(uint32 *)&buf[1]);
	SecurityNum = ntohs(*(uint16 *)&buf[5]);
	OpMode = 1;

	Board_Mng_Set_User_Num(CmdId, OpMode, UserNum);
#else

	Board_Mng_Set_User_Num(1, 1, 0x1234, 0x567);
