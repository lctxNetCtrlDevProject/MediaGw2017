...
#define INC_API_MODULE		/* ensure that inlinable API functions will be built in lock.h if required */
...
#define INC_API_MODULE		/* ensure that inlinable API functions will be built in this module if required */
...
INC_THREADSTORAGE(inet_ntoa_buf, inet_ntoa_buf_init);
...
INC_MUTEX_DEFINE_STATIC(__mutex);
				...
				int *h_errnop) 
{
	int hsave;
	struct hostent *ph;
	inc_mutex_lock(&__mutex); /* begin critical area */
	hsave = h_errno;
	...
	*h_errnop = h_errno; /* copy h_errno to *h_herrnop */
	if (ph == NULL) {
		*result = NULL;
		...
		if (nbytes > buflen) {
			*result = NULL;
			inc_mutex_unlock(&__mutex); /* end critical area */
		...
		*q++ = NULL; /* address list terminator */
		...
		for (p = ph->h_aliases; *p != 0; p++) {
			strncpy((char *)pbuf, *p, sizeof(pbuf));
			*q++ = pbuf; /* the pointer is the one inside buf... */
		...
		*q++ = NULL; /* terminator */
		...
		strncpy((char *)pbuf, ph->h_name, sizeof(pbuf));
		ret->h_name = pbuf;
...
struct hostent *inc_gethostbyname(const char *host, struct inc_hostent *hp)
{
	...
	if (!s || !*s) {
		...
		if (dots != 3)
			return NULL;
		memset(hp, 0, sizeof(struct inc_hostent));
...
{
	return inc_base64encode_full(dst, src, srclen, max, 0);
}
...
/*! \brief  inc_uri_encode: Turn text string to URI-encoded %XX version
...
char *inc_uri_encode(const char *string, char *outbuf, int buflen, int doreserved) 
{
	...
	inc_copy_string(outbuf, string, buflen);
...
/*! \brief  inc_uri_decode: Decode SIP URI, URN, URL (overwrite the string)  */
...
/*! \brief  inc_inet_ntoa: Recursive thread safe replacement of inet_ntoa */
const char *inc_inet_ntoa(struct in_addr ia)
{
	char *buf;

	if (!(buf = inc_threadstorage_get(&inet_ntoa_buf, INET_ADDRSTRLEN)))
		return "";
...
#ifndef __linux__
#undef pthread_create /* For inc_pthread_create function only */
#endif /* !__linux__ */
...
struct thr_lock_info {
	 ...
	 * Intentionally not inc_mutex_t */
	pthread_mutex_t lock;
	INC_LIST_ENTRY(thr_lock_info) entry;
};

/*! 
 * \brief Locked when accessing the lock_infos list 
 */
INC_MUTEX_DEFINE_STATIC(lock_infos_lock);
/*!
 * \brief A list of each thread's lock info 
 */
static INC_LIST_HEAD_NOLOCK_STATIC(lock_infos, thr_lock_info);
...
static void lock_info_destroy(void *data)
{
	...
	INC_LIST_REMOVE(&lock_infos, lock_info, entry);
 ...
 */
INC_THREADSTORAGE_CUSTOM(thread_lock_info, thread_lock_info_init, lock_info_destroy);
	...
	int line_num, const char *func, const char *lock_name, void *lock_addr)
{
	...
	if (!(lock_info = inc_threadstorage_get(&thread_lock_info, sizeof(*lock_info))))
		return;
	...
	if (lock_info->num_locks == INC_MAX_LOCKS) {
		/* Can't use inc_log here, because it will cause infinite recursion */
		fprintf(stderr, "XXX ERROR XXX A thread holds more locks than '%d'."
			"  Increase INC_MAX_LOCKS!\n", INC_MAX_LOCKS);
		pthread_mutex_unlock(&lock_info->lock);
		return;
	}
...
int inc_find_lock_info(void *lock_addr, char *filename, size_t filename_size, int *lineno, char *func, size_t func_size, char *mutex_name, size_t mutex_name_size)
{
	...
	inc_copy_string(filename, lock_info->locks[i].file, filename_size);
	*lineno = lock_info->locks[i].line_num;
	inc_copy_string(func, lock_info->locks[i].func, func_size);
	inc_copy_string(mutex_name, lock_info->locks[i].lock_name, mutex_name_size);
...
static const char *locktype2str(enum inc_lock_type type)
{
	switch (type) {
	case INC_MUTEX:
		return "MUTEX";
...
{
	struct thr_lock_info *lock_info;
	struct inc_dynamic_str *str;

	if (!(str = inc_dynamic_str_create(4096)))
		return RESULT_FAILURE;

	inc_dynamic_str_append(&str, 0, "\n" 
	            "=======================================================================\n"
	...
	pthread_mutex_lock(&lock_infos_lock.mutex);
	INC_LIST_TRAVERSE(&lock_infos, lock_info, entry) {
		int i;
		if (lock_info->num_locks) {
			inc_dynamic_str_append(&str, 0, "=== Thread ID: %ld (%s)\n", (long) lock_info->thread_id,
				lock_info->thread_name);
			pthread_mutex_lock(&lock_info->lock);
			for (i = 0; str && i < lock_info->num_locks; i++) {
				...
				inc_dynamic_str_append(&str, 0, "=== ---> %sLock #%d (%s): %s %d %s %s %p (%d)\n", 
				...
				if (lock_info->locks[i].type != INC_MUTEX)
					continue;

				lock = lock_info->locks[i].lock_addr;

				inc_reentrancy_lock(lock);
				for (j = 0; str && j < lock->reentrancy; j++) {
					inc_dynamic_str_append(&str, 0, "=== --- ---> Locked Here: %s line %d (%s)\n",
						lock->file[j], lock->lineno[j], lock->func[j]);
				}
				inc_reentrancy_unlock(lock);	
			}
			pthread_mutex_unlock(&lock_info->lock);
			if (!str)
				break;
...
struct thread_list_t {
	INC_LIST_ENTRY(thread_list_t) list;
	char *name;
	pthread_t id;
};

static INC_LIST_HEAD_STATIC(thread_list, thread_list_t);

void inc_register_thread(char *name)
{ 
	struct thread_list_t *new = inc_calloc(1, sizeof(*new));

	if (!new)
		return;
	new->id = pthread_self();
	new->name = name; /* steal the allocated memory for the thread name */
	INC_LIST_LOCK(&thread_list);
	INC_LIST_INSERT_HEAD(&thread_list, new, list);
	INC_LIST_UNLOCK(&thread_list);
}
...
void inc_unregister_thread(void *id)
{
	...
	INC_LIST_LOCK(&thread_list);
	INC_LIST_TRAVERSE_SAFE_BEGIN(&thread_list, x, list) {
		if ((void *) x->id == id) {
			INC_LIST_REMOVE_CURRENT(&thread_list, list);
			break;
		}
	}
...
static void *dummy_start(void *data)
{
	...
	/* note that even though data->name is a pointer to allocated memory,
	   we are not freeing it here because inc_register_thread is going to
	   keep a copy of the pointer and then inc_unregister_thread will
	   free the memory
	*/
	free(data);
	inc_register_thread(a.name);
	pthread_cleanup_push(inc_unregister_thread, (void *) pthread_self());
	...
	pthread_mutexattr_settype(&mutex_attr, INC_MUTEX_KIND);
	pthread_mutex_init(&lock_info->lock, &mutex_attr);
	pthread_mutexattr_destroy(&mutex_attr);

	pthread_mutex_lock(&lock_infos_lock.mutex); /* Intentionally not the wrapper */
	INC_LIST_INSERT_TAIL(&lock_infos, lock_info, entry);
			     ...
			     int line, const char *start_fn)
{
	...
	if ((errno = pthread_attr_setinheritsched(attr, PTHREAD_INHERIT_SCHED)))
		inc_log(LOG_WARNING, "pthread_attr_setinheritsched: %s\n", strerror(errno));
#endif

	if (!stacksize)
		stacksize = INC_STACKSIZE;

	if ((errno = pthread_attr_setstacksize(attr, stacksize ? stacksize : INC_STACKSIZE)))
		inc_log(LOG_WARNING, "pthread_attr_setstacksize: %s\n", strerror(errno));

#if !defined(LOW_MEMORY)
	if ((a = inc_malloc(sizeof(*a)))) {
		a->start_routine = start_routine;
		a->data = data;
		start_routine = dummy_start;
		if (asprintf(&a->name, "%-20s started at [%5d] %s %s()",
			     start_fn, line, file, caller) < 0) {
			inc_log(LOG_WARNING, "asprintf() failed: %s\n", strerror(errno));
			a->name = NULL;
		}
		data = a;
	}
	...
	return pthread_create(thread, attr, start_routine, data); /* We're in inc_pthread_create, so it's okay */
}
...
int inc_carefulwrite(int fd, char *s, int len, int timeoutms) 
{
	...
	while (len) {
		...
		while ((res = inc_poll(&pfd, 1, timeoutms - elapsed)) <= 0) {
			if (res == 0) {
				/* timed out. */
				if (option_debug) {
					inc_log(LOG_DEBUG, "Timed out trying to write\n");
				}
				return -1;
			} else if (res == -1) {
				...
				if (errno == EINTR || errno == EAGAIN) {
					elapsed = inc_tvdiff_ms(inc_tvnow(), start);
				...
				/* Fatal error, bail. */
				inc_log(LOG_ERROR, "poll returned error: %s\n", strerror(errno));
		...
		if (res < 0 && errno != EAGAIN && errno != EINTR) {
			...
			inc_log(LOG_ERROR, "write() returned error: %s\n", strerror(errno));
			return -1;
		}
...
char *inc_strip_quoted(char *s, const char *beg_quotes, const char *end_quotes)
{
	char *e;
	char *q;
...
char *inc_unescape_semicolon(char *s)
{
	char *e;
	char *work = s;
...
int inc_build_string_va(char **buffer, size_t *space, const char *fmt, va_list ap)
{
	int result;
...
int inc_build_string(char **buffer, size_t *space, const char *fmt, ...)
{
	va_list ap;
	int result;

	va_start(ap, fmt);
	result = inc_build_string_va(buffer, space, fmt, ap);
	va_end(ap);

	return result;
}

int inc_true(const char *s)
{
	if (inc_strlen_zero(s))
		return 0;
...
static struct timeval tvfix(struct timeval a)
{
	if (a.tv_usec >= ONE_MILLION) {
		inc_log(LOG_WARNING, "warning too large timestamp %ld.%ld\n",
			a.tv_sec, (long int) a.tv_usec);
		a.tv_sec += a.tv_usec / ONE_MILLION;
		a.tv_usec %= ONE_MILLION;
	} else if (a.tv_usec < 0) {
		inc_log(LOG_WARNING, "warning negative timestamp %ld.%ld\n",
			a.tv_sec, (long int) a.tv_usec);
		a.tv_usec = 0;
	}
	return a;
...
INC_MUTEX_DEFINE_STATIC(randomlock);

long int inc_random(void)
{
	long int res;
	inc_mutex_lock(&randomlock);
	res = random();
	inc_mutex_unlock(&randomlock);
	return res;
}
#endif
...
void inc_join(char *s, size_t len, char * const w[])
{
	int x, ofs = 0;
	const char *src;
...
const char __inc_string_field_empty[] = "";

static int add_string_pool(struct inc_string_field_mgr *mgr, size_t size)
{
	struct inc_string_field_pool *pool;

	if (!(pool = inc_calloc(1, sizeof(*pool) + size)))
		return -1;
...
int __inc_string_field_init(struct inc_string_field_mgr *mgr, size_t size,
			    inc_string_field *fields, int num_fields)
{
	int index;
	...
	for (index = 0; index < num_fields; index++)
		fields[index] = __inc_string_field_empty;

	return 0;
}

inc_string_field __inc_string_field_alloc_space(struct inc_string_field_mgr *mgr, size_t needed,
						inc_string_field *fields, int num_fields)
{
	char *result = NULL;
...
void __inc_string_field_index_build_va(struct inc_string_field_mgr *mgr,
				       inc_string_field *fields, int num_fields,
...
void __inc_string_field_index_build(struct inc_string_field_mgr *mgr,
				    inc_string_field *fields, int num_fields,
				    int index, const char *format, ...)
{
	...
	__inc_string_field_index_build_va(mgr, fields, num_fields, index, format, ap1, ap2);

	va_end(ap1);
	va_end(ap2);
}

INC_MUTEX_DEFINE_STATIC(fetchadd_m); /* used for all fetc&add ops */

int inc_atomic_fetchadd_int_slow(volatile int *p, int v)
{
        int ret;
        inc_mutex_lock(&fetchadd_m);
        ret = *p;
        *p += v;
        inc_mutex_unlock(&fetchadd_m);
        return ret;
}
 ...
 */
int inc_get_time_t(const char *src, time_t *dst, time_t _default, int *consumed)
{
	long t;
	int scanned;
...
int inc_dynamic_str_thread_build_va(struct inc_dynamic_str **buf, size_t max_len,
	struct inc_threadstorage *ts, int append, const char *fmt, va_list ap)
{
	int res;
	int offset = (append && (*buf)->len) ? strlen((*buf)->str) : 0;
#if defined(DEBUG_THREADLOCALS)
	struct inc_dynamic_str *old_buf = *buf;
	...
	if ((res + offset + 1) > (*buf)->len && (max_len ? ((*buf)->len < max_len) : 1)) {
		...
		if (!(*buf = inc_realloc(*buf, (*buf)->len + sizeof(*(*buf)))))
			return INC_DYNSTR_BUILD_FAILED;
								...
								struct inc_threadstorage *ts, const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	res = inc_dynamic_str_thread_set_va(buf, max_len, ts, fmt, ap);
	va_end(ap);
...
void inc_enable_packet_fragmentation(int sock)
{
	...
	if (setsockopt(sock, IPPROTO_IP, IP_MTU_DISCOVER, &val, sizeof(val)))
		inc_log(LOG_WARNING, "Unable to disable PMTU discovery. Large UDP packets may fail to be delivered when sent from this socket.\n");
...
int _inc_asprintf(char **ret, const char *file, int lineno, const char *func, const char *fmt, ...)
{
	int res;
	va_list ap;
