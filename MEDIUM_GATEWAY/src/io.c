 ...
 */
struct io_rec {
	inc_io_cb callback;		/* What is to be called */
...
struct io_context *io_context_create(void)
{
	...
	if ((tmp = inc_malloc(sizeof(*tmp)))) {
		tmp->needshrink = 0;
		tmp->fdcnt = 0;
		tmp->maxfdcnt = GROW_SHRINK_SIZE/2;
		tmp->current_ioc = -1;
		if (!(tmp->fds = inc_calloc(1, (GROW_SHRINK_SIZE / 2) * sizeof(*tmp->fds)))) {
			free(tmp);
			tmp = NULL;
			return NULL;
		} else {
			if (!(tmp->ior = inc_calloc(1, (GROW_SHRINK_SIZE / 2) * sizeof(*tmp->ior)))) {
				free(tmp->fds);
				free(tmp);
				tmp = NULL;
				return NULL;
...
static int io_grow(struct io_context *ioc)
{
	 ...
	 */
	void *tmp;
	DEBUG(inc_log(LOG_DEBUG, "io_grow()\n"));
	ioc->maxfdcnt += GROW_SHRINK_SIZE;
	if ((tmp = inc_realloc(ioc->ior, (ioc->maxfdcnt + 1) * sizeof(*ioc->ior)))) {
		ioc->ior = tmp;
		if ((tmp = inc_realloc(ioc->fds, (ioc->maxfdcnt + 1) * sizeof(*ioc->fds)))) {
			ioc->fds = tmp;
		} else {
			/*
...
int *inc_io_add(struct io_context *ioc, int fd, inc_io_cb callback, short events, void *data)
{
	/*
	 ...
	 */
	int *ret;
	DEBUG(inc_log(LOG_DEBUG, "inc_io_add()\n"));
	...
	if (!(ioc->ior[ioc->fdcnt].id = inc_malloc(sizeof(*ioc->ior[ioc->fdcnt].id)))) {
...
int *inc_io_change(struct io_context *ioc, int *id, int fd, inc_io_cb callback, short events, void *data)
{
	if (*id < ioc->fdcnt) {
		if (fd > -1)
			ioc->fds[*id].fd = fd;
...
int inc_io_remove(struct io_context *ioc, int *_id)
{
	int x;
	if (!_id) {
		inc_log(LOG_WARNING, "Asked to remove NULL?\n");
		return -1;
	}
	...
	inc_log(LOG_NOTICE, "Unable to remove unknown id %p\n", _id);
	return -1;
}

int inc_io_wait(struct io_context *ioc, int howlong)
{
	/*
	...
	int x;
	int origcnt;
	DEBUG(inc_log(LOG_DEBUG, "inc_io_wait()\n"));
	res = inc_poll(ioc->fds, ioc->fdcnt, howlong);
	if (res > 0) {
		/*
		 * At least one event
		 */
			...
			if (ioc->fds[x].revents && ioc->ior[x].id) {
				...
				if (ioc->ior[x].callback) {
					if (!ioc->ior[x].callback(ioc->ior[x].id, ioc->fds[x].fd, ioc->fds[x].revents, ioc->ior[x].data)) {
						...
						inc_io_remove(ioc, ioc->ior[x].id);
					}
				}
				ioc->current_ioc = -1;
			}
...
void inc_io_dump(struct io_context *ioc)
{
	/*
	 ...
	 */
	int x;
	inc_log(LOG_DEBUG, "INC_CC IO Dump: %d entries, %d max entries\n", ioc->fdcnt, ioc->maxfdcnt);
	inc_log(LOG_DEBUG, "================================================\n");
	inc_log(LOG_DEBUG, "| ID    FD     Callback    Data        Events  |\n");
	inc_log(LOG_DEBUG, "+------+------+-----------+-----------+--------+\n");
	for (x = 0; x < ioc->fdcnt; x++) {
		inc_log(LOG_DEBUG, "| %.4d | %.4d | %p | %p | %.6x |\n", 
				*ioc->ior[x].id,
				ioc->fds[x].fd,
				ioc->ior[x].callback,
				ioc->ior[x].data,
...
int inc_restore_tty(int fd, int oldstate)
{
	int res;
	struct termios tios;
	if (oldstate < 0)
